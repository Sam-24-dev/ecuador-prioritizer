...
function exportCsv(session: { results: SessionResultItem[] }) {
  const header = ['orden', 'clase_preliminar', 'puntaje_falso', 'p_verdadero', 'fuente', 'texto'];
  const lines = session.results.map((item, index) => [escapeCsvCell(index + 1, { trustedGeneratedNumber: true }), escapeCsvCell(item.preliminary_class), escapeCsvCell(item.score_false, { trustedGeneratedNumber: true }), escapeCsvCell(item.p_true, { trustedGeneratedNumber: true }), escapeCsvCell(item.source), escapeCsvCell(item.text)].join(','));
...
export function ResultadosPage() {
  ...
  const falseCount = session.results.filter((item) => item.preliminary_class === 'Falso').length;
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <section className="max-w-3xl border-l-2 border-terracotta py-2 pl-5"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Resultados priorizados</p><h1 className="mt-3 text-balance font-editorial text-4xl font-semibold leading-[1.02] tracking-tight sm:text-5xl">Orden de atención para este lote.</h1><p className="mt-4 text-base leading-relaxed text-muted-foreground"><span className="font-mono tabular-nums">{session.results.length}</span> publicaciones · <span className="font-mono tabular-nums">{falseCount}</span> con clasificación preliminar Falso · analizado el {analyzedAt}.</p></section>
      <Card className="mt-10"><CardHeader className="gap-5 border-b border-border sm:flex-row sm:items-end sm:justify-between"><div><CardTitle>Prioridad de revisión</CardTitle><CardDescription className="mt-2 max-w-2xl">Las noticias se ordenan para ayudarte a decidir cuál revisar primero.</CardDescription></div><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" onClick={() => exportCsv(session)}><Download className="h-4 w-4" aria-hidden="true" />Descargar todos los resultados (CSV)</Button><Button type="button" variant="ghost" onClick={reset}><RotateCcw className="h-4 w-4" aria-hidden="true" />Borrar resultados</Button></div></CardHeader>
        <CardContent className="space-y-0 pt-2 sm:pt-3">{session.results.map((item, index) => <article key={item.id} className="grid gap-5 border-b border-border py-6 last:border-b-0 lg:grid-cols-[4rem_minmax(0,1fr)_12rem]"><div className="font-mono text-xl font-semibold tabular-nums text-primary">{String(index + 1).padStart(2, '0')}</div><div className="min-w-0 space-y-5"><ScoreIndicator scoreFalse={item.score_false} preliminaryClass={item.preliminary_class} /><div><p className="mb-1.5 text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Fuente</p>{(item.displaySource ?? item.source) ? <HttpSource source={item.displaySource ?? item.source ?? ''} /> : <p className="text-sm text-muted-foreground">Sin fuente indicada</p>}</div><details className="border border-border bg-muted/40 p-4 text-sm"><summary className="min-h-11 cursor-pointer rounded px-2 py-2.5 font-semibold text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Ver texto analizado</summary><p className="mt-4 whitespace-pre-wrap break-words leading-relaxed">{item.text}</p></details></div><dl className="grid grid-cols-2 gap-3 self-start text-sm lg:grid-cols-1"><div><dt className="text-xs text-muted-foreground">Puntaje falso</dt><dd className="mt-1 font-mono font-semibold tabular-nums">{(item.score_false * 100).toFixed(1)}%</dd></div><div><dt className="text-xs text-muted-foreground">Puntaje verdadero</dt><dd className="mt-1 font-mono font-semibold tabular-nums">{(item.p_true * 100).toFixed(1)}%</dd></div></dl></article>)}</CardContent>
      </Card>
    </div>
  );
}
