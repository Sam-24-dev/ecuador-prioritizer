...
The public product is a stateless web client backed by an owner-managed scoring service. The confirmed public frontend is the Cloudflare Worker at <https://ecuador-prioritizer.scaizapa.workers.dev/>. The FastAPI backend on Render is publicly reachable by this application, but it is not a supported public API or third-party integration.

```mermaid
flowchart LR
    U[Anonymous browser] --> W[Cloudflare Worker frontend]
    W --> A[Owner-managed Render FastAPI service]
...
| Owner-managed FastAPI service on Render | Validate bounded requests and run scoring; serve the public application | Supported third-party API, editorial workflow, accounts, or background jobs |
...
3. The owner-managed FastAPI service validates shape and size, computes features, runs the scoring pipeline, and returns ranked, non-definitive output.
...
|---|---|
| Approved source, documentation, verified public datasets, and material explicitly cleared for release | Scraped Ecuador data, raw source content, credentials, operational logs, non-public runtime details, model/data bundles, snapshots, archives, and unapproved legacy material |
